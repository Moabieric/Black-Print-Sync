...
require_once BP_COMMERCE_PATH
    . 'projection/adoption/ControlledOwnershipAdopter.php';

use BlackPrint\Commerce\Projection\Adoption\ControlledOwnershipAdopter;

echo '<pre>';

echo "\n";
echo "==========================================================\n";
echo "BLACKPRINT OS — STEP 5.1\n";
echo "CONTROLLED OWNERSHIP ADOPTION — DRY RUN\n";
echo "==========================================================\n";

$ownershipAdopter = new ControlledOwnershipAdopter();

$ownershipResult = $ownershipAdopter->run(
    $adoptionMappings,
    $verifiedWooCommerceProducts,
    $verifiedCanonicalProducts,
    $verifiedCanonicalVariants,
    true // DRY RUN — NO WRITES
);

echo "\n";
echo "VERIFICATION SCOPE\n";
echo "----------------------------------------------------------\n";
echo "Verified WooCommerce products:      "
    . count($verifiedWooCommerceProducts) . "\n";

echo "Verified canonical products:        "
    . count($verifiedCanonicalProducts) . "\n";

echo "Verified canonical variants:        "
    . count($verifiedCanonicalVariants) . "\n";

echo "\n";
echo "PRODUCT OWNERSHIP\n";
echo "----------------------------------------------------------\n";
echo "Products examined:                  "
    . $ownershipResult['products_examined'] . "\n";

echo "Products requiring adoption:        "
    . $ownershipResult['products_requiring_adoption'] . "\n";

echo "Products already correctly owned:   "
    . $ownershipResult['products_already_correctly_owned'] . "\n";

echo "\n";
echo "VARIANT OWNERSHIP\n";
echo "----------------------------------------------------------\n";
echo "Variants examined:                  "
    . $ownershipResult['variants_examined'] . "\n";

echo "Variants requiring adoption:        "
    . $ownershipResult['variants_requiring_adoption'] . "\n";

echo "Variants already correctly owned:   "
    . $ownershipResult['variants_already_correctly_owned'] . "\n";

echo "\n";
echo "SAFETY BOUNDARIES\n";
echo "----------------------------------------------------------\n";
echo "REVIEW products touched:            "
    . $ownershipResult['review_products_touched'] . "\n";

echo "DO NOT ADOPT products touched:      "
    . $ownershipResult['do_not_adopt_products_touched'] . "\n";

echo "Unverified variants touched:        "
    . $ownershipResult['unverified_variants_touched'] . "\n";

echo "Incomplete-family variants added:   "
    . $ownershipResult['incomplete_family_variants_added'] . "\n";

echo "Mappings created:                   "
    . $ownershipResult['mappings_created'] . "\n";

echo "WooCommerce products created:       "
    . $ownershipResult['products_created'] . "\n";

echo "WooCommerce variations created:     "
    . $ownershipResult['variations_created'] . "\n";

echo "WooCommerce products deleted:       "
    . $ownershipResult['products_deleted'] . "\n";

echo "WooCommerce variations deleted:     "
    . $ownershipResult['variations_deleted'] . "\n";

echo "\n";
echo "OWNERSHIP CONFLICTS\n";
echo "----------------------------------------------------------\n";
echo "Conflicts:                          "
    . $ownershipResult['ownership_conflicts'] . "\n";

echo "\n";
echo "ERRORS\n";
echo "----------------------------------------------------------\n";
echo "Errors:                             "
    . count($ownershipResult['errors']) . "\n";

if ($ownershipResult['errors'] !== []) {
    echo "\n";
    echo "ERROR SAMPLES:\n";

    foreach (array_slice($ownershipResult['errors'], 0, 20) as $error) {
        echo wp_json_encode(
            $error,
...
if ($ownershipResult['conflict_samples'] !== []) {
    echo "\n";
    echo "CONFLICT SAMPLES:\n";

    foreach (
        array_slice($ownershipResult['conflict_samples'], 0, 20)
        as $conflict
    ) {
        echo wp_json_encode(
            $conflict,
...
if ($ownershipResult['metadata_anomaly_samples'] !== []) {
    echo "\n";
    echo "METADATA ANOMALY SAMPLES:\n";

    foreach (
        array_slice($ownershipResult['metadata_anomaly_samples'], 0, 20)
        as $anomaly
    ) {
        echo wp_json_encode(
            $anomaly,
...
echo "\n";
echo "STATUS: " . $ownershipResult['status'] . "\n";
